...
// document has no wording of its own -- see siren.Entity.Label.
//
...
// one here.
func Document(entity *siren.Entity, fallbackTitle string) RenderedDocument {
	if entity == nil {
	...
	rows = append(rows, entityRows(*entity)...)
	rows = append(rows, linkRows(*entity)...)
